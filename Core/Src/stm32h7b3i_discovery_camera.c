...
/** @addtogroup STM32H7B3I_EVAL_CAMERA
  * @{
  */

/** @defgroup STM32H7B3I_EVAL_CAMERA_Exported_Variables CAMERA Exported Variables
  * @{
  */
...
/** @defgroup STM32H7B3I_EVAL_CAMERA_Private_Variables CAMERA Private Variables
  * @{
  */
...
/** @defgroup STM32H7B3I_EVAL_CAMERA_Private_FunctionsPrototypes CAMERA Private Functions Prototypes
  * @{
  */
...
/** @defgroup STM32H7B3I_EVAL_CAMERA_Exported_Functions CAMERA Exported Functions
  * @{
  */
...
int32_t BSP_CAMERA_Init(uint32_t Instance, uint32_t Resolution, uint32_t PixelFormat)
{
  ...
  else
  {
    ...
    else
    {
      ...
      io_init_structure.Pull = GPIO_PULLUP;
      io_init_structure.Mode = GPIO_MODE_INPUT;
...
int32_t BSP_CAMERA_DeInit(uint32_t Instance)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(BSP_CAMERA_PwrDown(Instance) != BSP_ERROR_NONE)
      {
        ret = BSP_ERROR_BUS_FAILURE;
      }
      else
...
int32_t BSP_CAMERA_HwReset(uint32_t Instance)
{
  ...
  else
  {
    ...
    io_init_structure.Pull = GPIO_NOPULL;
    io_init_structure.Mode = GPIO_MODE_OUTPUT_PP;
...
/** @defgroup STM32H7B3I_EVAL_CAMERA_Private_Functions CAMERA Private Functions
  * @{
  */
...
static int32_t S5K5CAG_Probe(uint32_t Resolution, uint32_t PixelFormat)
{
  ...
  S5K5CAG_IO_t              IOCtx;
  uint32_t                 id;
  static S5K5CAG_Object_t   S5K5CAGObj;
  ...
  if(S5K5CAG_RegisterBusIO (&S5K5CAGObj, &IOCtx) != S5K5CAG_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else if(S5K5CAG_ReadID(&S5K5CAGObj, &id) != S5K5CAG_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else
  {
    ...
    {
      Camera_Drv = (CAMERA_Drv_t *) &S5K5CAG_CAMERA_Driver;
      Camera_CompObj = &S5K5CAGObj;
      if(Camera_Drv->Init(Camera_CompObj, Resolution, PixelFormat) != S5K5CAG_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
	  else if(Camera_Drv->GetCapabilities(Camera_CompObj, Camera_Cap) != S5K5CAG_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
